...
void main() {
  ...
  vec2 pixelCoord = floor(fragCoord / pixelSize) * pixelSize + pixelSize * 0.5;
...
function DitheredWaves({
  ...
  // Keep resolution in sync (initial and on resize)
  useEffect(() => {
    const dpr = gl.getPixelRatio();
    const newWidth = Math.floor(size.width * dpr);
    const newHeight = Math.floor(size.height * dpr);
    const currentRes = waveUniformsRef.current.resolution
  ...
  useFrame(({ clock }) => {
    ...
    // Keep resolution in sync (handles first frame and resize)
    const dpr = gl.getPixelRatio();
